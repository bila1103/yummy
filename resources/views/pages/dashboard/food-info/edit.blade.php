...
@section('action')
  <a href="{{ route('dashboard.food-info.index') }}" class="btn btn-secondary">
...
@section('content')
  <div class="col-12 col-lg-8 mx-auto">
    <form id="form-ingredients" action="{{ route('dashboard.food-info.update', $foodInfo->id) }}" method="POST"
      class="card my-5" enctype="multipart/form-data">
      <div class="card-header border-bottom">
        <h5 class="mb-0">
          <i class="ri-edit-line"></i>
          <span class="fw-bold">Edit Info</span>
        </h5>
      </div>
      <div class="card-body">
        <div class="row g-3">
          <div class="col-12">
            <div class="mb-3">
              ...
              <input class="form-control @error('name') is-invalid @enderror rounded-0" name="name"
                value="{{ old('name', $foodInfo->name) }}" id="name" placeholder="Nama Bagian" type="text"
                required>
              @error('name')
            ...
            <div>
              ...
              <textarea rows="5" class="form-control @error('description') is-invalid @enderror rounded-0" name="description"
                id="description" placeholder="Deskripsi Bagian">{{ old('description', $foodInfo->description) }}</textarea>
              @error('description')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
